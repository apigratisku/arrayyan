...
class Sales extends CI_Controller {
    ...
    public function index() {
        if (!file_exists(APPPATH.'views/backend/sales/index.php')) {
            show_404();
        }
        ...
        $data['items'] = $this->m_sales->get();
       
		$this->load->view('backend/header', $data);
		$this->load->view('backend/sales/index', $data); 
        $this->load->view('backend/footer');
    }

    public function tambah() {
        if (!file_exists(APPPATH.'views/backend/sales/input.php')) {
            show_404();
        }
		$data['wo_all'] = $this->m_wo->count_wo_all();
		$data['wo_selesai'] = $this->m_wo->count_wo_selesai();
        ...
        ";
		$this->load->view('backend/header', $data);
        $this->load->view('backend/sales/input', $data);
    ...
    public function simpan() {
		...
		$data['wo_all'] = $this->m_wo->count_wo_all();
		$data['wo_selesai'] = $this->m_wo->count_wo_selesai();
		$data['wo_baru'] = $this->m_wo->count_wo_baru();		$data['jumlah_pelanggan'] = $this->m_data_pelanggan->count();
		$data['sch_total'] = $this->scheduler_model->sch_total();
		$data['sch_run'] = $this->scheduler_model->sch_run();
		$data['sch_wait'] = $this->scheduler_model->sch_wait();
    ...
    public function timpa($id) {
        ...
        $this->m_sales->timpa($id);
		redirect('manage/sales');
    }
	
	
    public function hapus($id) {
        if ($this->m_sales->delete($id)) {
