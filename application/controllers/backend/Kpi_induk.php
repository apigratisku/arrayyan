...
class Kpi_induk extends CI_Controller {
	...
	if($this->session->userdata('ses_admin')=='1' || $this->session->userdata('ses_admin')=='2' || $this->session->userdata('ses_admin')=='3' || $this->session->userdata('ses_admin')=='4' || $this->session->userdata('ses_admin')=='6'){
        if (!file_exists(APPPATH.'views/backend/kpi_induk/index.php')) {
            show_404();
        }
        ...
        $data['menu']  = 'kpi_induk';
        $data['title'] = 'Data KPI - Root Data';
        $data['items'] = $this->m_kpi_induk->get();
       
		$this->load->view('backend/header', $data);
		$this->load->view('backend/kpi_induk/index', $data); 
	...
	if($this->session->userdata('ses_admin')=='1' || $this->session->userdata('ses_admin')=='4'){
        if (!file_exists(APPPATH.'views/backend/kpi_induk/input.php')) {
            show_404();
        }
		$data['wo_all'] = $this->m_wo->count_wo_all();
		$data['wo_selesai'] = $this->m_wo->count_wo_selesai();
        ...
        ";
		$this->load->view('backend/header', $data);
        $this->load->view('backend/kpi_induk/input', $data);
	...
	if($this->session->userdata('ses_admin')=='1' || $this->session->userdata('ses_admin')=='4'){
        ...
        $data['item']  = $this->m_kpi_induk->get($id);
	...
	if($this->session->userdata('ses_admin')=='1' || $this->session->userdata('ses_admin')=='4'){
        ...
        $this->m_kpi_induk->timpa($id);
		redirect('manage/kpi_induk');
	}else{
	redirect('logout');
	}
    }
