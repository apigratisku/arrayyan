...
class Vas_spec extends CI_Controller {
	...
	if($this->session->userdata('ses_admin')=='1' ||  $this->session->userdata('ses_admin')=='2' ||  $this->session->userdata('ses_admin')=='3' ||  $this->session->userdata('ses_admin')=='5' ||  $this->session->userdata('ses_admin')=='6'){
        if (!file_exists(APPPATH.'views/backend/vas_spec/index.php')) {
            show_404();
        }
        ...
        $data['menu']  = 'vas_spec';
        $data['title'] = 'Data VAS Spesifikasi';
        $data['items'] = $this->m_vas_spec->get();
       
		$this->load->view('backend/header', $data);
		$this->load->view('backend/vas_spec/index', $data); 
	...
	if($this->session->userdata('ses_admin')=='1' ||  $this->session->userdata('ses_admin')=='3' ||  $this->session->userdata('ses_admin')=='5'){
        if (!file_exists(APPPATH.'views/backend/vas_spec/input.php')) {
            show_404();
        }
		$data['wo_all'] = $this->m_wo->count_wo_all();
		$data['wo_selesai'] = $this->m_wo->count_wo_selesai();
        ...
        ";
		$this->load->view('backend/header', $data);
        $this->load->view('backend/vas_spec/input', $data);
	...
	if($this->session->userdata('ses_admin')=='1' ||  $this->session->userdata('ses_admin')=='3' ||  $this->session->userdata('ses_admin')=='5'){
        ...
        $data['item']  = $this->m_vas_spec->get($id);
	...
	if($this->session->userdata('ses_admin')=='1' ||  $this->session->userdata('ses_admin')=='3' ||  $this->session->userdata('ses_admin')=='5'){
        ...
        $this->m_vas_spec->timpa($id);
		redirect('manage/vas_spec');
	}else{
	redirect('logout');
	}
    }
