...
class M_kpi_induk extends CI_Model {
    ...
    public function get($id=false) {
        ...
        if ($id) {
            $query = $this->db->get_where('blip_kpi_induk', array('id' => $id));
            $response = $query->row_array();
        } else {
            $this->db->select('*');
            $this->db->from('blip_kpi_induk');
			//$this->db->limit(1, 1);
    ...
    public function simpan() {
		...
		$insert_db = $this->db->insert('blip_kpi_induk', $data);
    ...
    public function timpa($id) {
      ...
      $data = array(
		...
		$this->db->where('id', $id);
		$update_db = $this->db->update('blip_kpi_induk', $data);			
    ...
    public function delete($id) {	
		...
		$this->db->insert('blip_log_user', $log);
		//End Log
        return $this->db->delete('blip_kpi_induk', array('id' => $id));
    }
