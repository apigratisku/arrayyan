    ...
    public function simpan() {
		...
		if($this->input->post('permintaan') == "Upgrade" or $this->input->post('permintaan') == "Downgrade")
		{
				...
				if ($this->routerosapi->connect("$xdark_h","$xdark_u","$xdark_p"))
				{
					...
					$this->routerosapi->write('=on-event='."/queue simple set [find target=".$routerid['router_network']."] max-limit=".$bw_convert."; /tool fetch url=\"https://".$aplikasi."/xdark/update.php?Res=".$resource."&Scheduler=".$this->input->post('idrouter')."&status=1\" keep-result=no; /system scheduler set [find name=\"".$schname."\"] comment=\"script-done\" disabled=yes;", false);					
					$this->routerosapi->write('=disabled=no');				
					$this->routerosapi->read();
					$this->routerosapi->disconnect();	
					$this->session->set_flashdata('message','Data berhasil ditambahkan!');
					$data = $this->telegram_lib->sendmsg($this->input->post('permintaan')." pelanggan <b>".$pelanggan."</b> telah dijadwalkan tanggal <b>".$newDate."</b> pukul <b>".$newTime."</b> secara otomatis ya kak. \n\nBerikut Detail Teknis: \nBW Sebelum: <b>".$bw_normal."</b> \nBW Sesudah: <b>".$this->input->post('bw')."</b>");
					}
					else
					{
					//Save to DB
					...
					$this->routerosapi->disconnect();	
					$this->session->set_flashdata('message','Data berhasil ditambahkan!');	
					$data = $this->telegram_lib->sendmsg($this->input->post('permintaan')." <b>".$pelanggan."</b> telah sukses di eksekusi tanggal <b>".$newDate3."</b> secara otomatis ya kak. \n\nBerikut Detail Teknis: \nBW Sebelum: <b>".$bw_normal."</b> \nBW Sesudah: <b>".$this->input->post('bw')."</b>");
					}
				}
				else
				{
		...
		{	
				...
				if($this->input->post('eksekusi') == "terjadwal")
				{
					...
					$this->routerosapi->disconnect();	
					$this->session->set_flashdata('message','Data berhasil ditambahkan!');
					$data = $this->telegram_lib->sendmsg("Isolir pelanggan <b>".$pelanggan."</b> telah dijadwalkan tanggal <b>".$newDate."</b> pukul <b>".$newTime."</b> secara otomatis ya kak.");
				...
				else
				{
					...
					$this->routerosapi->disconnect();	
					$this->session->set_flashdata('message','Data berhasil ditambahkan!');
					$data = $this->telegram_lib->sendmsg("Isolir pelanggan <b>".$pelanggan."</b> telah sukses di eksekusi tanggal <b>".$newDate3."</b> secara otomatis ya kak.");		
					}
					else
					{
						$this->session->set_flashdata('message', 'Login gagal. Pastikan hostname, username dan password yang Anda masukkan benar!');
		...
		{	
				...
				if($this->input->post('eksekusi') == "terjadwal")
				{
					...
					$this->routerosapi->write('=start-time='.$newTime, false);  
					$this->routerosapi->write('=comment='."Menunggu", false); 
					$this->routerosapi->write('=on-event='."/ip firewall filter set [find comment=\"".$routerid['router_network']." - ".$pelanggan."\"] comment=\"".$routerid['router_network']." - Open Isolir ".$pelanggan." - ".$newDate."\" disabled=yes; /tool fetch url=\"https://".$aplikasi."/xdark/update.php?Res=".$resource."&Scheduler=".$this->input->post('idrouter')."&status=1\" keep-result=no; /system scheduler remove [find name=\"".$schname."\"];", false);						
					$this->routerosapi->write('=disabled=no');				
					$this->routerosapi->read();
					$this->routerosapi->disconnect();	
					$this->session->set_flashdata('message','Data berhasil ditambahkan!');
					$data = $this->telegram_lib->sendmsg("Open isolir pelanggan <b>".$pelanggan."</b> telah dijadwalkan tanggal <b>".$newDate."</b> pukul <b>".$newTime."</b> secara otomatis ya kak");
					}
					else
					{
						$this->session->set_flashdata('message', 'Login gagal. Pastikan hostname, username dan password yang Anda masukkan benar!');
				...
				else
				{
					...
					$this->routerosapi->disconnect();	
					$this->session->set_flashdata('message','Data berhasil ditambahkan!');
					$data = $this->telegram_lib->sendmsg("Open isolir pelanggan <b>".$pelanggan."</b> telah sukses di eksekusi tanggal <b>".$newDate3."</b> secara otomatis ya kak.");		
					}
					else
					{
						$this->session->set_flashdata('message', 'Login gagal. Pastikan hostname, username dan password yang Anda masukkan benar!');
		...
		{
			//BOD (Bandwidth On Demand NORMAL)
					require_once APPPATH."third_party/addon.php";
					$kripto 	 = new Cipher(MCRYPT_BLOWFISH, MCRYPT_MODE_ECB);
					$xdark_rand  = "%^$%^&%*GMEDIA%^$%^&%*";
					$xdark_h 	 = $kripto->decrypt("Dkihc87sCnrjdUG9RF5LwA==", $xdark_rand);
					...
					$this->routerosapi->write('=start-time='.$newTime, false); 
					$this->routerosapi->write('=comment='."Menunggu", false); 
					$this->routerosapi->write('=on-event='."/queue simple set [find target=".$routerid['router_network']."] max-limit=".$bw_bod."; /system scheduler set [find name=\"".$schname."-mulai\"] disabled=yes comment=\"script-done\"; /tool fetch url=\"https://".$aplikasi."/xdark/update.php?Res=".$resource."&Scheduler=".$this->input->post('idrouter')."&status=2\" keep-result=no", false);					
					$this->routerosapi->write('=disabled=no');				
					$this->routerosapi->read();
					...
					$this->routerosapi->write('=comment='."Menunggu", false); 
					$this->routerosapi->write('=on-event='."/queue simple set [find target=".$routerid['router_network']."] max-limit=".$bw_bod."; /tool fetch url=\"https://".$aplikasi."/xdark/update.php?Res=".$resource."&Scheduler=".$this->input->post('idrouter')."&status=2\" keep-result=no; /system scheduler set [find name=\"".$schname."-mulai\"] comment=\"script-done\" disabled=yes;", false);					
					$this->routerosapi->write('=disabled=no');				
					$this->routerosapi->read();
					...
					$this->routerosapi->disconnect();	
					$this->session->set_flashdata('message','Data berhasil ditambahkan!');
					$data = $this->telegram_lib->sendmsg("BOD pelanggan <b>".$pelanggan."</b> telah sukses di eksekusi tanggal <b>".$newDate3."</b> secara otomatis ya kak. \n\nBerikut Detail Teknis: \nStart BOD: <b>".$newDate3."</b> \nFinish BOD: <b>".$newDate2." ".$inTime."</b> \nBW Sebelum: <b>".$bw_awal."</b> \nBW Sesudah: <b>".$bw_DB."</b>");
					}
				}
				else
				{
