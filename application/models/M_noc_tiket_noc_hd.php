...
class M_noc_tiket_noc_hd extends CI_Model {
    ...
    public function get($id=false) {
        ...
        if ($id) {
            $query = $this->db->get_where('blip_tiket_noc_hd', array('id' => $id));
            $response = $query->row_array();
        } else {
            $this->db->select('*');
            $this->db->from('blip_tiket_noc_hd');
			//$this->db->limit(1, 1);
    ...
    public function simpan() {
		...
		$insert_db = $this->db->insert('blip_tiket_noc_hd', $data);
    ...
    public function timpa($id) {
      ...
      $data = array(
		...
		$this->db->where('id', $id);
		$update_db = $this->db->update('blip_tiket_noc_hd', $data);			
    ...
    public function delete($id) {
		//Log Aktifitas
		$log = array(
			'aktifitas' => "Menghapus data NOC HD",
			'history_waktu' => date("Y-m-d H:i"),
			'history_iduser' => $this->session->userdata('ses_id'),
		);				
		$this->db->insert('blip_log_user', $log);
		//End Log	
        return $this->db->delete('blip_tiket_noc_hd', array('id' => $id));
    }
