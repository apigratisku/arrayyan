...
class Telegram_lib
{
	...
	public function sendblip($chatid,$msg)
    {
        $telegram = new Telegram($this->ci->config->item('bot_token', 'telegram'));
        $chat_id = $chatid;
        $content = array('chat_id' => $chat_id, 'text' => $msg, 'parse_mode' => "HTML");
	...
	public function sendptam($chatid,$msg)
    {
        $telegram = new Telegram($this->ci->config->item('bot_token_ptam', 'telegram'));
        $chat_id = $chatid;
        $content = array('chat_id' => $chat_id, 'text' => $msg, 'parse_mode' => "HTML");
        return $telegram->sendMessage($content);
    }
	...
	public function sendblip_doc($chatid,$doc_path, $caption = false)
    {
        $telegram = new Telegram($this->ci->config->item('bot_token', 'telegram'));
        $chat_id = $chatid;
        $content = ['chat_id' => $chat_id, 'document' => new CURLFile(realpath($doc_path)), 'caption' => $caption, 'parse_mode' => "HTML"];
        return $telegram->sendDocument($content);
    }
